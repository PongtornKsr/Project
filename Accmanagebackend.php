...
if(isset($_POST['nmsearch'])){
    ...
    if(isset($_POST['query'])){
            ...
            $query = "SELECT * FROM userdata natural join userstat natural join userprofile WHERE (givenName NOT IN ('$fname')) AND  (givenName LIKE '%$sch%' or familyName  LIKE '%$sch%' or email  LIKE '%$sch%') AND ID NOT IN ('47','54','55') AND ID_stat NOT IN ('2') AND name NOT IN ('".$_SESSION['Account']."')";
            $_SESSION['query'] = $query;
    }
    else{
        
            $query ="SELECT * FROM userdata natural join userstat natural join userprofile  WHERE givenName NOT IN ('$fname') and givenName NOT IN ('$ffname') AND ID NOT IN ('47','54','55') AND ID_stat NOT IN ('2') AND name NOT IN ('".$_SESSION['Account']."')";
            $_SESSION['query'] = $query;
            $datethai = [];
    }
    ...
    if(mysqli_num_rows($result) > 0)
    {
      ...
      if($row['profile_ID']==2){ $output.= " <button type='button' data-func='5' value = '".$row['ID']."' id = 'role_up' class='btn btn-info'>ตั้งเป็นเจ้าหน้าที่</button>";}                       
...
if(isset($_POST['sort'])){
  ...
  while($row = mysqli_fetch_array($result)){
    ...
    $datethai = explode("/",$row['last_update']);
      ...
      if($row['profile_ID']==2){ $output.= " <button type='button' value = '".$row['ID']."'  data-func='5' id = 'role_up' class='btn btn-info'>ตั้งเป็นเจ้าหน้าที่</button>";}                       
